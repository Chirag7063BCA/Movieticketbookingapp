...
const Navbar = () => {
  ...
  return (
   ...
   <div className={`max-md:absolute max-md:top-0 max-md:left-0 max-md:font-medium max-md:text-lg z-50 flex flex-col 
      ...
      bg-black/70 md:bg-white/10 md:border border-gray-300/20 overflow-hidden transition-[width] duration-300 ${isOpen ? 'max-md:w-full':'max-md:w-0'}`}> <img src={assets.logo} alt ="" className='w-36 h-auto'/> </div>
    </Link>
    <div className={`max-md:absolute max-md:top-0 max-md:left-0 max-md:font-medium max-md:text-lg z-50 flex flex-col 
      md:flex-row items-center max-md:justify-center gap-8 min-md:px-8 py-3 max-md:h-screen min-md:rounded-full backdrop-blur 
    ...
    <XIcon className='md:hidden absolute top-6 rigth-6 w-6 h-6 cursor-pointer' onClick={()=>setIsOpen(!isOpen)}/>
